...
module.exports = async (message, options) => {
    ...
    generalEmbed.fields[0].value = `Cartes: ${yourcards.map(c => `[\`${c.emoji} ${c.rank}\`](https://discord.gg/xkebY6nsxk)`).join(" ")}\nTotal:${isSoft ? " Soft" : ""} ${yourcards.map(c => c.value).reduce((a, b) => b + a)}`
    generalEmbed.fields[1].value = `‎`
    generalEmbed.fields[2].value = `Cartes: [\`${dealercards[0].emoji} ${dealercards[0].rank}\`](https://discord.gg/xkebY6nsxk) \` ? \`\nTotal: \` ? \``
